...
export default function TeacherVolunteerListPage() {
  ...
  useEffect(() => {
    // 募集中のボランティアのみ表示
    const activeActivities = activities.filter((activity) => activity.status === "募集中")
      ...
      <main className="px-6 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <h1 className="text-2xl font-bold text-slate-900">ボランティア一覧（先生用）</h1>
            <div className="w-20"></div> {/* Spacer for centering */}
          </div>

          {/* Info Card */}
          <div className="bg-blue-50/60 backdrop-blur-sm rounded-2xl p-6 border border-blue-100 shadow-sm mb-8">
            <h3 className="text-lg font-semibold text-blue-800 mb-3">先生用ボランティア管理</h3>
            <p className="text-blue-700 text-sm mb-2">こちらでは主催者が募集したボランティア活動を確認できます。</p>
          ...
          <div className="mb-8">
            <p className="text-slate-600">
              ...
              <span className="ml-4 text-sm">
                （共有済み: {volunteerActivities.filter((a) => a.sharedByTeacher).length}件）
              </span>
            </p>
          </div>
            ...
            {volunteerActivities.map((activity) => (
              ...
              >
                ...
                <div className="space-y-3 flex-grow flex flex-col">
                  ...
                  <p className="text-sm text-slate-600 line-clamp-2 flex-grow">{activity.description}</p>
